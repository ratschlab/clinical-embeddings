...
def parse_arguments() -> argparse.Namespace:
    ...
    parser_compute.add_argument("-m", "--memory", type=int, default=4,
        help="Requests {m}G of memory per core")
    parser_compute.add_argument("-t", "--time", type=int, default=1,
        help="Request {t} hours of job duration")
    parser_compute.add_argument('-e', '--exclude', default=[], nargs='+', help="Nodes to exclude from submission")
...
def create_slurm_command(args: argparse.Namespace, compute_config: dict = {}) -> str:
    ...
    gpu_type = "" if compute_config['gpus'] is None else f":{compute_config['gpu_type']}"
    gpu_cmd = ""
    ...
    with open(final_config_path, 'w') as f:
        ...
        for h_key, h_value in sorted(config.items()):
            ...
            if h_key in {"modality_splitting", "mod_split", "mod_split_clustered"}:
                ...
                split_class = "ClusteredSplittedEmbedding" if "cluster" in h_key else "Splitted_Embedding"
...
def config_to_cmd_str(config: dict, args: argparse.Namespace, split_seeds: bool = False, multi_gpu_flag: bool = False) -> str:
...
def run_on_slurm(config: dict, slurm_command: str, args: argparse.Namespace, multi_gpu_flag: bool = False) -> int:
    
    config_cmds = config_to_cmd_str(config, args, split_seeds=True, multi_gpu_flag=multi_gpu_flag)
