...
from icu_benchmarks.common.constants import (DATETIME, PID, VALUE, VARID,
                                             VARREF_LOWERBOUND,
                                             VARREF_UPPERBOUND)
from icu_benchmarks.common.datasets import Dataset
from icu_benchmarks.preprocessing.preprocess_pharma import (
    convert_cumul_value_to_rate, drop_duplicates_pharma,
...
def drop_out_of_range_values(df, varref):
    ...
    df_with_bounds = df.merge(
        varref[bound_cols], left_on=VARID, right_index=True, how="inner"
    )
...
def aggregate_cols(wide_observ, varref):
    metavar_varid_dict = {
        m: [f"v{vid}" for vid in vars]
        for (m, vars) in varref.reset_index().groupby("metavariableid")["variableid"]
    }
    ...
    for vmid, varid_cols in metavar_varid_dict.items():
        ...
        if len(varid_cols_avail) > 1:
            c = wide_observ.loc[:, varid_cols_avail].median(
...
def transform_obs_table_fn(
    observ: pd.DataFrame, lst_vmid: List[int], lst_cumul_vid, varref, general_table
):
    ...
    wide_observ = pd.pivot_table(
        observ, values=VALUE, columns=VARID, index=DATETIME
    ).sort_index()
    ...
    binary_vmids = [
        "vm%d" % x
        for x in varref[
            varref.metavariableunit.apply(lambda x: x == "Binary")
        ].metavariableid.values
    ]
    for col in binary_vmids:
        wide_aggregated.loc[:, col] = wide_aggregated[col].apply(
            lambda x: x if np.isnan(x) else float(x != 0)
        )
...
def transform_pharma_table_fn(pharma: pd.DataFrame, pharmaref, lst_pmid):
    ...
    pharma.drop(
        pharma.index[pharma.recordstatus.isin([522, 526, 546, 782])], inplace=True
    )
    ...
    if pharma.empty:
        ...
        for pharmaid in pharma.pharmaid.unique():
            pharma_acting_period = (
                pharmaref[pharmaref.pharmaid == pharmaid].iloc[0].pharmaactingperiod_min
            )
            infusion_rate = []
            for infusionid in pharma[pharma.pharmaid == pharmaid].infusionid.unique():
                tmp_pharma = pharma[
                    (pharma.pharmaid == pharmaid) & (pharma.infusionid == infusionid)
                ].copy()
                infusion_rate.append(
                    process_single_infusion(tmp_pharma, pharma_acting_period)
                )
        ...
        for pmid in lst_pmid:
            cols = [
                "p%d" % x for x in pharmaref[pharmaref.metavariableid == pmid].pharmaid
            ]
            if np.isin(wide_pharma.columns, cols).sum() == 0:
                wide_pharma.loc[:, "pm%d" % pmid] = np.nan
            else:
                if (
                    pharmaref[pharmaref.metavariableid == pmid]
                    .unitconversionfactor.notnull()
                    .sum()
                    > 0
                ):
                    unitconverters = [
                        pharmaref[
                            (pharmaref.metavariableid == pmid)
                            & (pharmaref.pharmaid == int(c[1:]))
                        ]
                        .iloc[0]
                        .unitconversionfactor
                        for c in wide_pharma.columns[np.isin(wide_pharma.columns, cols)]
                    ]
                    wide_pharma.loc[:, "pm%d" % pmid] = (
                        wide_pharma[
                            wide_pharma.columns[np.isin(wide_pharma.columns, cols)]
                        ]
                        * unitconverters
                    ).sum(axis=1)
                else:
        ...
        binary_pmids = [
            "pm%d" % x
            for x in pharmaref[
                pharmaref.metavariableunit.apply(lambda x: x == "Binary")
            ].metavariableid.values
        ]
        for col in binary_pmids:
            wide_pharma.loc[:, col] = wide_pharma[col].apply(
                lambda x: x if np.isnan(x) else float(x != 0)
            )
...
def combine_obs_and_pharma_tables(
    dfs: Sequence[Mapping[int, pd.DataFrame]], columns, admission_times
):
    ...
    for pid in all_ids:
        ...
        df_pid[PID] = pid
        df_pid = df_pid.reset_index().rename(
            columns={"index": DATETIME, "givenat": DATETIME}
        )
...
def merge_tables(
    ...
    lst_cumul_vid = varref[
        varref.variablename.apply(lambda x: "/c" in x.lower() or "cumul" in x.lower())
    ].index.tolist()

    obs_per_pat = functools.partial(
        transform_obs_table_fn,
        lst_vmid=lst_vmid,
        lst_cumul_vid=lst_cumul_vid,
        varref=varref,
        general_table=general_table,
    )
    transform_pharma_table_fn_per_pat = functools.partial(
        transform_pharma_table_fn, pharmaref=pharmaref, lst_pmid=lst_pmid
    )
    ...
    output_cols = (
        [PID, DATETIME]
        + [f"vm{vid}" for vid in sorted(varref["metavariableid"].unique())]
        + [f"pm{vid}" for vid in sorted(pharmaref["metavariableid"].unique())]
    )

    admission_times = {
        pid: adm_time for (pid, adm_time) in general_table["admissiontime"].items()
    }
