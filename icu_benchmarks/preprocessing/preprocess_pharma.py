...
from icu_benchmarks.common.constants import DATETIME, PID
...
def drop_duplicates_pharma(df):
    ...
    for pharmaid in df_dup.pharmaid.unique():
        for infusionid in df_dup[df_dup.pharmaid == pharmaid].infusionid.unique():
            tmp = df_dup[
                (df_dup.pharmaid == pharmaid) & (df_dup.infusionid == infusionid)
            ]
            if (
                len(tmp.recordstatus.unique()) == 1
                and tmp.recordstatus.unique()[0] == 780
            ):
                for i in range(len(tmp)):
                    df.loc[tmp.index[i], "infusionid"] = "%s_%s" % (
                        int(df.loc[tmp.index[i], "infusionid"]),
                        i,
                    )
                ...
                len(tmp.recordstatus.unique()) == 1
                and tmp.recordstatus.unique()[0] == 776
            ):
                ...
                len(tmp.recordstatus.unique()) == 2 and 776 in tmp.recordstatus.unique()
            ):
...
def process_single_infusion(df, acting_period):
    ...
    for sub_infusionid in df.infusionid.unique():
        ...
        try:
            ...
            pass
        tmp.loc[:, "rate"] = 0
        tmp.loc[tmp.index[:-1], "rate"] = (
            tmp.givendose.values[1:]
            / (tmp.givenat.diff() / np.timedelta64(1, "m")).values[1:]
        )
...
def convert_cumul_value_to_rate(df, cumul_urine_id_lst, general_table):
    ...
    if df[df.variableid == 200]["value"].notnull().sum() > 0:
        hr_first_meas_time = df.loc[
            df[df.variableid == 200]["value"].notnull().index[0], DATETIME
        ]
    ...
    if len(df_urine) == 0:
        ...
        for vid in df_urine.variableid.unique():
            df_tmp = df_urine[
                df_urine.variableid == vid
            ]  # table of a single urine variable

            index_pre_general_table = df_tmp.index[
                df_tmp[DATETIME] < esti_adm_time - np.timedelta64(15 * 60 + 30, "s")
            ]
            ...
            df_tmp = df[df.variableid == vid]
            t_reset = df_tmp[
                (df_tmp["value"].diff() < 0) | (df_tmp.index == df_tmp.index[0])
            ][
                DATETIME
            ]  # reset time for the cumulative counting
            for i in np.arange(1, len(t_reset)):
                tmp = df_tmp[df_tmp[DATETIME] >= t_reset.iloc[i]]
                if i < len(t_reset) - 1:
                    tmp = tmp[tmp[DATETIME] < t_reset.iloc[i + 1]]
                df.loc[tmp.index, "value"] += df.loc[
                    df_tmp.index[df_tmp[DATETIME] < t_reset.iloc[i]][-1], "value"
                ]
        ...
        for vid in df_urine.variableid.unique():
            ...
            if len(df_tmp) == 0:
                ...
                df.loc[df_tmp.index[1:], "value"] = (
                    df_tmp["value"].diff().iloc[1:] / tdiff.iloc[1:]
                ).values
