...
def process_chunk(part_path: Path, output_dir: Path):
    ...
    impute_one_batch.execute(
        batch_id=batch_id, merged_path=merged_path, imputed_path=imputed_path
    )
...
def impute_for_endpoints(merged_path, output_dir, nr_workers=1):
    ...
    processing.exec_parallel_on_parts(
        functools.partial(process_chunk, output_dir=output_dir), parts, nr_workers
    )
