...
def extract_feature_df(df, df_var_ref):
    ...
    df_feat = df.groupby(constants.PID).apply(
        lambda x: extract_feat_patient(x, exclude=to_exclude)
    )
    df_feat = df_feat.reset_index(level=0, drop=True)
    return df_feat


def extract_feat_patient(patient_sample, exclude, extract_presence=True):
    cols = [
        c
        for c in patient_sample.columns
        if c not in exclude + [constants.PID, constants.DATETIME]
    ]
    ...
    patient_feat[meas_cols] = (
        pd.notna(patient_sample[cols]).astype(float).cumsum().ffill().values
    )
    patient_feat[mean_cols] = (
        patient_sample[cols].cumsum().ffill().values / patient_feat[meas_cols].values
    )

    if extract_presence:
        # We make presence for all
        all_feat_cols = [
            c
            for c in patient_sample.columns
            if c not in [constants.PID, constants.DATETIME]
        ]
        presence_cols = ["presence_" + c for c in all_feat_cols]
        patient_feat[presence_cols] = (
            patient_sample[all_feat_cols].notnull().astype(float)
        )
        return patient_feat[
            [constants.PID, constants.DATETIME]
            + min_cols
            + max_cols
            + mean_cols
            + meas_cols
