...
def process_endpoint_chunk(
    part_path: Path, imputed_path: Path, merged_path: Path, output_dir: Path
):

    # modify config
    batch_id = int(re.match("part-([0-9]+).parquet", part_path.name).groups()[0])

    endpoint_benchmark.endpoint_gen_benchmark(
        batch_id=batch_id,
        endpoint_path=output_dir,
