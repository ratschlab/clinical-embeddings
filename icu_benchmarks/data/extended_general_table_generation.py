...
from icu_benchmarks.common.constants import PID, VALUE, VARID
...
def generate_extended_general_table(
    observation_tables_path, general_table_path, output_path
):
    ...
    df_additional_cols = pd.pivot_table(
        df_dropped, index=PID, columns=VARID, values=VALUE
    ).rename(
        columns={
            10000450: "height",
