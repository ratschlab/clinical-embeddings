...
def process_chunk(
    part_path: Path,
    endpoints_path: Path,
    imputation_for_endpoints_path: Path,
    output_dir: Path,
    static_path: Path,
    horizon,
):
    ...
    label_benchmark.label_gen_benchmark(
        batch_id=batch_id,
        label_path=output_dir,
        endpoint_path=endpoints_path,
        imputed_path=imputation_for_endpoints_path,
...
def generate_labels(
    endpoints_path: Path,
    imputation_for_endpoints_path: Path,
    exended_general_data_path: Path,
    output_dir: Path,
    nr_workers=1,
    horizon=12,
):
    ...
    processing.exec_parallel_on_parts(
        functools.partial(
            process_chunk,
            endpoints_path=endpoints_path,
            imputation_for_endpoints_path=imputation_for_endpoints_path,
            output_dir=output_dir,
            static_path=exended_general_data_path,
            horizon=horizon,
        ),
        parts,
        nr_workers,
