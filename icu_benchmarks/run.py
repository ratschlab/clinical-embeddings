...
from icu_benchmarks.data import imputation_for_endpoints, extended_general_table_generation, endpoint_generation, \
    labels, schemata
from icu_benchmarks.data.preprocess import to_ml, merge_multiple_horizon_labels, get_utf8_cols
...
def build_parser():
    ...
    model_arguments.add_argument('-mod_split', '--modality_splitting', default=None,
                                 dest="modality_splitting", required=False, nargs='+',
                                 type=str,
    ...
    model_arguments.add_argument('-el', '--embedding-latent', default=None,
                                 dest="embedding_latent", required=False, nargs='+',
                                 type=int,
                                 help="Latent dim of the embedding module")
    #FTT params
    model_arguments.add_argument('-self_attention_dim_emb', '--self-attention-dim-emb', default=None, 
                                 dest="self_attention_dim_emb", required=False, nargs='+', type=int,
                                 help="Hidden dim of token in FTT")

    model_arguments.add_argument('-heads_emb', '--heads-emb', default=None, 
                                 dest="heads_emb", required=False, nargs='+', type=int,
                                 help="Number of heads in Sel-Attention layer in FTT")

    model_arguments.add_argument('-mult_emb', '--mult-emb', default=None,
                                 dest="mult_emb", required=False, nargs='+', type=int,
                                 help="Dimension of fully-connected layer in Transformer block in FTT")

    #Transformer aggregation params
    model_arguments.add_argument('-do_agg', '--do-agg', default=None, dest="do_agg",
                                 required=False, nargs='+', type=float,
                                 help="Dropout probability for the Transformer block in Transformer aggregation")
    model_arguments.add_argument('-do_att_agg', '--do-att-agg', default=None, dest="do_att_agg",
                                 required=False, nargs='+', type=float,
                                 help="Dropout probability for the Self-Attention layer only in Transformer aggregation")
    model_arguments.add_argument('-self_attention_dim_agg', '--self-attention-dim-agg', default=None, dest="self_attention_dim_agg",
                                 required=False, nargs='+', type=int,
                                 help="Dimension in Transformer aggregation")

    model_arguments.add_argument('-mult_agg', '--mult-agg', default=None,
                                 dest="mult_agg", required=False, nargs='+',
                                 type=int,
                                 help="Dimension of fully-connected layer in Transformer block in Transformer aggregation")
    model_arguments.add_argument('-heads_agg', '--heads-agg', default=None,
                                 dest="heads_agg", required=False, nargs='+',
                                 type=int,
                                 help="Number of heads in Sel-Attention layer in Transformer aggregation")

    model_arguments.add_argument('-emb_merge', '--embedding-merge', default=None,
                                 dest="embedding_merge", required=False, nargs='+',
                                 type=str,
    ...
    model_arguments.add_argument('--aux-task', default=False,
                                 dest="aux_task", required=False, type=bool,
                                 help="Whether to do auxiliary tasks")
    model_arguments.add_argument('--aux-weight', default=None, type=float,
                                 dest="aux_weight", required=False, nargs='+',
                                 help="Weight for auxiliary loss")
    model_arguments.add_argument('--aux-type', default=None, type=str,
                                 dest="aux_type", required=False, nargs='+',
...
def run_build_ml(common_path, labels_path, features_path: Optional[Path], ml_path, var_ref_path,
    ...
    if new_ml_stage or not all_path.exists():
        ...
        radicals_files = ['_'.join(data_path.parts[-1].split('_')[:-1]) for data_path in tables_path]
        tables_path = [t for i, t in enumerate(tables_path) if radicals_files[i] == radical_file]
        horizons = [int(data_path.parts[-1].split('_')[-1].rstrip('Hours.h5')) for i, data_path in
                    enumerate(tables_path)]
...
def main(my_args=tuple(sys.argv[1:])):
    ...
    if args.command == 'preprocess':
        run_preprocessing_pipeline(args.hirid_data_root, args.work_dir, args.var_ref_path,
                                   ...
                                   scaler=args.scaler, clipping=args.clipping, horizon=args.horizon)
