...
from icu_benchmarks.common.constants import (APACHE_2_MAP, APACHE_2_NAME,
                                             APACHE_4_MAP, APACHE_4_NAME,
                                             CIRC_FAILURE_NAME, DATETIME,
                                             DISCHARGE_NAME, HR_CUM_NAME,
                                             LOS_NAME, MORTALITY_NAME,
                                             PHENOTYPING_NAME, PID,
                                             REL_DATETIME, RESP_FAILURE_NAME,
                                             STEPS_PER_HOUR, URINE_BINARY_NAME,
                                             URINE_CUM_NAME, URINE_REG_NAME,
                                             VAR_IDS_EP)
...
def gen_label(
    df_pat, df_endpoint, horizon, mort_status=None, apache_group=None, pid=None
):
    ...
    dynamic_mort_arr = utils.unique_label_at_hours(
        stay_length, mort_status, at_hours=24
    )
    ...
    urine_reg_arr, urine_binary_arr = utils.future_urine_output(
        urine_col, urine_meas_arr, weight_col, rhours=2
    )
...
def label_gen_benchmark(
    batch_id, label_path, endpoint_path, imputed_path, static_path, horizon
):
    ...
    cand_files = glob.glob(
        os.path.join(endpoint_path, "batch_{}.parquet".format(batch_id))
    )
    ...
    for pidx, pid in enumerate(all_pids):
        ...
        apache_pat_group = utils.merge_apache_groups(
            apache_ii_group, apache_iv_group, apache_ii_map, apache_iv_map
        )
        ...
        if df_pat.shape[0] == 0 or df_endpoint.shape[0] == 0:
            if df_pat.shape[0] == 0:
                ...
                logging.info(
                    "WARNING: Empty imputed data in patient {}".format(pid), flush=True
                )
        ...
        if (pidx + 1) % 100 == 0:
            logging.info(
                "Progress for batch {}: {:.2f} %".format(
                    batch_id, (pidx + 1) / len(all_pids) * 100
                )
            )
