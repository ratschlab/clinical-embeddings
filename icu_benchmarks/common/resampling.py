...
def resample_df(df, freq_string="5T"):
    ...
    def reorder_time(patient_sample):
        ...
        if len(HRs_non_zero) > 0:
            ...
            patient_sample.loc[:HR_start_idx] = patient_sample.loc[
                :HR_start_idx
            ].ffill()
        else:
            HR_start_idx, HR_stop_idx = 0, patient_sample.shape[0] - 1
        stay_stop_time, stay_start_time = (
            patient_sample.loc[HR_stop_idx, constants.DATETIME],
            patient_sample.loc[HR_start_idx, constants.DATETIME],
        )
        patient_sample = patient_sample.loc[HR_start_idx:HR_stop_idx].reset_index(
            drop=True
        )
        offset = (
            np.timedelta64(stay_start_time.minute, "m")
            + np.timedelta64(stay_start_time.second, "s")
            + np.timedelta64(stay_start_time.microsecond, "us")
            + np.timedelta64(1, "us")
        )
        patient_sample.loc[:, constants.DATETIME] = (
            patient_sample[constants.DATETIME] - offset
        )
        grided = (
            patient_sample.set_index(constants.DATETIME)
            .resample(freq_string, axis=0, closed="left", label="right")
            .last()
            .reset_index()
        )
        grided.loc[:, constants.DATETIME] -= (
            stay_start_time - offset + np.timedelta64(1, "us")
        )
    ...
    df_part = df_part[
        [constants.PID] + [c for c in df_part.columns if c != constants.PID]
    ]
...
def rename_to_human_df(df, df_var_ref):
    ...
    names = [
        str(df_var_ref[df_var_ref.metavariableid == i]["metavariablename"].values[0])
        for i in meta_ids
    ]
    final_names = [
        to_rename[i] if names[i] == "nan" else names[i] for i in range(len(names))
    ]
