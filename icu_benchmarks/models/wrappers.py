...
from icu_benchmarks.models.metrics import BalancedAccuracy, MAE, CalibrationCurve, CustomAveragePrecision, \
    CustomROCAUC
...
gin.config.external_configurable(torch.nn.functional.binary_cross_entropy_with_logits, module='torch.nn.functional')
...
class DLWrapper(object):
    def __init__(self,
                 encoder=gin.REQUIRED, optimizer_fn=gin.REQUIRED,
                 reg='l1', reg_weight=1e-3,
                 cluster_reg=0.0,
                 aux_label_idx=None, aux_label_type='max',
                 aux_label_horizon=144,
                 aux_label_weight=1.0, lr_decay=1.0,
        ...
        if torch.cuda.is_available():
            ...
            self.n_worker = 4
            logging.info(f'Model will be trained using GPU Hardware [Accelerate], workers: {self.n_worker}')
    ...
    def set_metrics(self, allow_distributed: bool = True):
        ...
        if encoder.logit.out_features == 2:
            ...
            self.metrics = {
                'PR': CustomAveragePrecision(accelerator=self.accelerator, allow_distributed=allow_distributed),
                'AUC': CustomROCAUC(accelerator=self.accelerator, allow_distributed=allow_distributed)}
        ...
        if self.aux_label_idx is not None:
            self.aux_metrics = {'MAE_{}'.format(str(idx)): MeanAbsoluteError() for idx in self.aux_label_idx}
        else:
            self.aux_metrics = {}
    ...
    def get_auxiliary_labels(self, data):
        ...
        with torch.no_grad():
            padded_batch = torch.nn.functional.pad(data_4d[..., IDXS], (0, 0, HORIZON - 1, 0), mode='replicate')[0]
    ...
    def loss_fn(self, output, label, loss_weight):
        ...
        if self.task_type == 'regression':  # len(label.shape) == 2:
            ...
            return torch.nn.functional.cross_entropy(output, label.long(), weight=loss_weight)
        else: # multi-horizon
            return torch.nn.functional.binary_cross_entropy_with_logits(output, label, weight=loss_weight)
    ...
    def step_fn(self, element, loss_weight=None):
        ...
        if self.aux_label_idx is not None:
            ...
            aux_out_flat = torch.masked_select(aux_out, mask.unsqueeze(-1)).reshape(-1, aux_out.shape[-1])
            aux_label_flat = torch.masked_select(aux_labels, mask.unsqueeze(-1)).reshape(-1, aux_out.shape[-1])
            aux_loss = self.aux_label_weight * torch.nn.functional.mse_loss(aux_out_flat, aux_label_flat.float())
            aux_op = [aux_out_flat, aux_label_flat]
        else:
            aux_op = []
            aux_loss = 0.0

        if self.task_type != 'multioutput_binary_classification':  # len(labels.shape) <= 2:
            ...
            label_flat = torch.masked_select(labels, mask.unsqueeze(-1))  # .reshape(-1, labels.shape[-1])
        ...
        if len(out_flat) == 0:
            ...
            loss = torch.nan_to_num(self.loss_fn(out_flat, label_flat, loss_weight)) + aux_loss
        ...
        if self.reg_fn is not None:
            ...
            loss += self.reg_weight * self.reg_fn(self.accelerator.unwrap_model(self.encoder).embedding_layer)

        clustering_loss = 0
        if self.cluster_reg > 0:
            unwrapped_encoder_embedding = self.accelerator.unwrap_model(self.encoder).embedding_layer
            cluster_layer = unwrapped_encoder_embedding.clustering
            
            feature_tokenzier = unwrapped_encoder_embedding.feature_tokenizer
            feature_matrix = unwrapped_encoder_embedding.extract_embeddings_for_clustering(feature_tokenzier)
            ...
            if self.clustering_regularizer_type == "functional_hard":
                cl_assignments_matrix = torch.zeros((num_feature_vectors, num_clusters), device=feature_matrix.device)
                cl_assignments_matrix[np.arange(num_feature_vectors), cl_assignments] = 1
                clustering_loss_unscaled = cluster_layer.regularization(feature_matrix, cl_assignments_matrix)

            elif self.clustering_regularizer_type == "functional_soft":
                clustering_loss_unscaled = cluster_layer.soft_regularization(feature_matrix)

            else:
                raise NotImplementedError(f"Clustering regularizer type not implemented: {self.clustering_regularizer_type}")
    ...
    def _do_training(self, train_loader, weight, metrics):
        ...
        train_loss = []
        self.encoder.train()
        for t, elem in tqdm(enumerate(train_loader), total=len(train_loader), disable=not self.accelerator.is_local_main_process):
            ...
            if self.cluster_splitting and self.accelerator.is_main_process:
                ...
                cluster_layer = unwrapped_encoder_embedding.clustering
                feature_tokenzier = unwrapped_encoder_embedding.feature_tokenizer
                feature_matrix = unwrapped_encoder_embedding.extract_embeddings_for_clustering(feature_tokenzier).detach()
                # print(f"Feature Matrix: {feature_matrix}")

                new_assignments, updated_centroids = cluster_layer(
                    feature_matrix, prev_assignments=unwrapped_encoder_embedding.clustering_assignments)
                ...
                if self.clustering_tensorboard is not None:
                    ...
                    self.clustering_logger.add_scalar('cluster/inertia', cluster_inertia, self.clustering_logger_steps)
                    self.clustering_logger.add_scalar('cluster/ratio', cluster_ratio, self.clustering_logger_steps)
                    self.clustering_logger.add_scalar('cluster/updates', self.cluster_updates, self.clustering_logger_steps)
                    self.clustering_logger.add_scalar('cluster/loss', loss_cluster, self.clustering_logger_steps)

                    cluster_size_map = {str(i): size for i, size in enumerate(unwrapped_encoder_embedding.clustering_split_sizes)}
                    self.clustering_logger.add_scalars('cluster/sizes', cluster_size_map, self.clustering_logger_steps)

                    if self.clustering_logger_steps % self.clustering_plot_steps == 0:
                        cluster_plot = cluster_layer.plot_clusters(feature_matrix)
                        self.clustering_logger.add_figure('cluster/plot', cluster_plot, self.clustering_logger_steps)
        ...
        train_loss = torch.sum(torch.cat([t.unsqueeze(0) for t in train_loss])) / (t + 1)
    ...
    def train(self, train_dataset, val_dataset, weight,
        ...
        if self.cluster_splitting and self.clustering_tensorboard is not None:
            ...
            if self.accelerator.is_main_process:
                self.clustering_logger = SummaryWriter(os.path.join(self.logdir, 'tensorboard', 'cluster'))
            else:
        ...
        self.encoder, self.optimizer, train_loader, val_loader, self.scheduler = self.accelerator.prepare(
        ...
        if self.cluster_splitting:
            ...
            unw_enc_embs.clustering_assignments = unw_enc_embs.clustering_assignments.to(self.accelerator.device)
        ...
        for epoch in range(epochs):
            ...
            if val_loss <= best_loss - min_delta:
                ...
                epoch_no_improvement = 0
                if save_weights:
                    self.save_weights(epoch, os.path.join(self.logdir, f'model_e{epoch}.torch'), delete_previous=True)
                if self.accelerator.is_main_process:
                ...
                if self.accelerator.is_main_process:
                    logging.info(f'No improvement on loss for {epoch_no_improvement} epochs')

            if epoch_no_improvement >= patience:
                if self.accelerator.is_main_process:
                    logging.info(f'No improvement on loss for more than {patience} epochs. We stop training')
                break
        ...
        with self.accelerator.main_process_first():
            self.load_weights(glob.glob(os.path.join(self.logdir, 'model*.torch'))[0])  # We load back the best iteration
        self.accelerator.free_memory()
        self.accelerator.wait_for_everyone()

    def test(self, dataset, weight, test_filename='test_metrics.pkl'):

        self.set_metrics(allow_distributed=False)
        ...
        if isinstance(weight, list):
            weight = torch.FloatTensor(weight).to(self.device)
        test_loss, test_metrics = self.evaluate(test_loader, self.metrics, weight, distributed=False)
    ...
    def evaluate(self, eval_loader, metrics, weight, distributed: bool = True):
        ...
        with torch.no_grad():
            for v, elem in tqdm(enumerate(eval_loader), total=len(eval_loader), disable=not self.accelerator.is_local_main_process):
    ...
    def load_weights(self, load_path):
        logging.info(f"[PROC {self.accelerator.process_index}] load weights: {load_path}")
        unwrapped_encoder = self.accelerator.unwrap_model(self.encoder)
