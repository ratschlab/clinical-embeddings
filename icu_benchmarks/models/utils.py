    ...
    if modality_splitting in all_mods.keys():
        ...
        for group in groups_list:
            ...
            for variable_idx in group:
                ...
                if variable[2] is None or variable[2] == 1: # for numericals and binary cats.
...
def get_bindings_and_params(args):
    ...
    if args.embedding_latent:
        if args.rs:
            embedding_latent = args.embedding_latent[np.random.randint(len(args.embedding_latent))]
        else:
    ...
    if args.self_attention_dim_emb:
        if args.rs:
            self_attention_dim_emb = args.self_attention_dim_emb[np.random.randint(len(args.self_attention_dim_emb))]
        else:
    ...
    if args.self_attention_dim_agg:
        if args.rs:
            self_attention_dim_agg = args.self_attention_dim_agg[np.random.randint(len(args.self_attention_dim_agg))]
        else:
    ...
    if args.modality_splitting:
        if args.rs:
            modality_splitting = args.modality_splitting[np.random.randint(len(args.modality_splitting))]
        else:
            modality_splitting = args.modality_splitting[0]
        gin_bindings += ['MODALITY_SPLITTING = ' + str(get_modalities_list(modality_splitting))]

        log_dir = log_dir + '_modsplit_' + modality_splitting.replace('_','')

    if args.embedding_merge:
        if args.rs:
            embedding_merge = args.embedding_merge[np.random.randint(len(args.embedding_merge))]
        else:
